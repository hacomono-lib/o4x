...
func (s *OutboxRepositorySuite) TestReviveStuckPublishing_MarksAsDeadWhenMaxAttemptsReached() {
	...
	_, err = s.pool.Exec(ctx,
		"UPDATE outbox SET attempt_count = 2, updated_at = NOW() - INTERVAL '10 minutes' WHERE id = $1",
		msg.ID)
	s.Require().NoError(err)
...
func (s *OutboxRepositorySuite) TestReviveStuckPublishing_HandlesMultipleMessagesCorrectly() {
	...
	_, err = s.repo.FetchAndLockToPublishing(ctx)
	s.Require().NoError(err)
	_, err = s.pool.Exec(ctx, "UPDATE outbox SET attempt_count = 2 WHERE id = $1", msg2.ID)
	s.Require().NoError(err)

	// Simulate both stuck
	_, err = s.pool.Exec(ctx, "UPDATE outbox SET updated_at = NOW() - INTERVAL '10 minutes' WHERE id IN ($1, $2)",
		msg1.ID, msg2.ID)
	s.Require().NoError(err)
